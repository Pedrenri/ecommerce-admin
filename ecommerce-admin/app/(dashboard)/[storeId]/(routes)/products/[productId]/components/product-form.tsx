...
const formSchema = z.object({
  ...
  colorId: z.string().min(1, "Cor é obrigatória"),
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      console.log(error);
      toast.error(`Houve um erro ao salvar o produto. ${error}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error("Erro ao excluir este produto. Tente novamente mais tarde.");
    } finally {
      setLoading(false);
      setOpen(false);
    }
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <FormDescription>
                      Este Produto aparecerá na página principal
                    </FormDescription>
                  </div>
                </FormItem>
              )}
