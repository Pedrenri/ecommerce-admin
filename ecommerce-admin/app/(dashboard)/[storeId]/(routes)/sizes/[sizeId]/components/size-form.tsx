...
const formSchema = z.object({
  ...
  value: z.string().min(1, "Valor do tamanho é obrigatório"),
});
...
export const SizeForm: React.FC<SizeFormProps> = ({
  ...
  const title = initialData ? "Editar Tamanho" : "Novo Tamanho";
  const description = initialData
    ? "Editar um Tamanho"
    : "Criar um novo Tamanho";
  const toastMessage = initialData
    ? "Tamanho editado com sucesso"
  ...
  const onSubmit = async (data: SizeFormValues) => {
    try {
      ...
      console.log(error);
      toast.error(`Houve um erro ao salvar o tamanho. ${error}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error(
        "Erro ao excluir o tamanho. Verifique se há algum produto utilizando este tamanho."
      );
    } finally {
      setLoading(false);
      setOpen(false);
